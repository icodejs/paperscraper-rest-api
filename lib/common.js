...
exports.jsonpify = function (obj, callback) {
  var output = _.rtrim(obj.json, ', ');

  if (!callback) return obj.callbackId + '(' + output + ')';

  if (obj.isJsonp && obj.callbackId.length > 0) {
    callback(obj.callbackId + '(' + output + ')');
  } else {
    callback(output);
  }
};
